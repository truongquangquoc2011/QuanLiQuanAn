...
namespace QuanLyQuanAn_DongPhongNha.Admin
{
    public partial class frmAdminCategory : Form
    {
        ...
        private void txtCategoryName_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                e.Handled = true;
                errorProvider1.SetError(txtCategoryName, "Chỉ được nhập chữ, không được nhập số hoặc ký tự đặc biệt!");
            }
            else
            {
        ...
        private void btnAddCategory_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    errorProvider1.SetError(txtCategoryName, "Vui lòng nhập đầy đủ thông tin");
                    check = false;
                }
            }
            catch (Exception ex)
        ...
        {
            QLNHEntities db = new QLNHEntities();
            int max = db.FoodCategory.Max(n => n.idFC) + 1;
            txtCategoryID.Text = max.ToString();
        }
    }
}
